...
namespace ConsoleApplication2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            byte rad=5;    //Promień od jakiego należy tworzyć płatki
            //
            ...
            for(int i=0;i<otoczenie.getPiecesNumber();i++)  //Inicjalizacja każdego płatka wraz z nadaniem pozycji startowej
            {
                ...
                pozycja_startowa[0] = random.Next(0, (int)radius) * Math.Pow(-1.0, (double)(random.Next(1, 2)));    //Losowanie współrzędnej x płatka z zakresu od 0 do R z losowym znakiem
                pozycja_startowa[1] = SceneHeight;
                pozycja_startowa[2] = Math.Sqrt(Math.Pow((double)random.Next(rad, (int)radius), 2.0) - Math.Pow(pozycja_startowa[0], 2.0));  //wyznaczanie trzeciej współrzędnej płatka (z równania  okręgu o promieniu losowanym z przedziału <r,R> i środku (0,0)) z losowym znakiem
                platki[i]=new SnowFlake(i,pozycja_startowa,0.001, 0.00000312);
            }
