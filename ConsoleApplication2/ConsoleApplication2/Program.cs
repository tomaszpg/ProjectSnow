    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for(int i=0;i<otoczenie.getPiecesNumber();i++)  //Inicjalizacja każdego płatka wraz z nadaniem pozycji startowej
            {
                ...
                pozycja_startowa[2] = Math.Sqrt(Math.Pow((double)random.Next(0, (int)radius), 2.0) - Math.Pow(pozycja_startowa[0], 2.0));  //wyznaczanie trzeciej współrzędnej płatka (z równania  okręgu o promieniu losowanym z przedziału <r,R> i środku (0,0)) z losowym znakiem
